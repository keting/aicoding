...
本规范面向多人协作场景：项目负责人或核心成员负责拆解任务、分配工作和进行 PR review，项目成员负责认领任务、开发实现和提交 PR。

目标不是增加流程负担，而是让任务边界、责任归属、开发过程和验收标准足够清晰，避免重复劳动和无效 review。

本文中提到的“项目负责人”，在执行层面可由项目负责人指定的核心成员代行；若未特别区分，默认两者共享 review、任务确认和归并裁决职责。

## 适用范围

本文优先适用于开发类协作仓库，如 `half`。这类仓库应严格执行本文的最小规则和默认工作流；标签体系以仓库现状为准，不为了对齐文档再造体系。

对于私有但多人协作的仓库，可以沿用同样流程，但 review 严格度和标签体系可以适度简化。

对于以内容整理为主的公开协作仓库，如 `aicoding`，可以做以下调整：

- 最小规则第 `1`、`3`、`4` 条可豁免，小幅内容修订可直接提交 PR，不强制先开 issue，也不强制 branch 名带 issue 编号
- 最小规则第 `5`、`6` 条保留
- review 与合并流程仍按本文执行
...
4. `PR` 必须关联 `issue`
5. 一个 `PR` 只解决一个明确问题
6. `CI` 通过后才允许合并；新仓库若暂未建立 CI，先按口头约束执行，有余力后补齐 CI 配置并结合 branch protection 的 `Require status checks to pass` 机械化
...
8. 建议在 `CI` 通过后进入正式 review；项目负责人/核心成员在 3 个工作日内给出首轮 review
...
默认约定：项目负责人/核心成员在 3 个工作日内给出首轮 review；如果暂时无法 review，也应简短回复当前状态，避免项目成员长时间等待。
...
如果当前仓库已经有既定标签体系，文档应服从仓库现状，不要再造第二套。
...
在 `half` 这类项目中，文档类改动通常使用 `area:docs` 作为模块标签，同时仍按任务性质补充 `type:*`，例如 `type:maintenance`、`type:feature`。不建议额外新增 `type:docs` 这类标签。
...
如果仓库已经启用 branch protection，建议同时开启 `Require status checks to pass`，把“CI 通过后才允许合并”从口头规则变成系统约束。
